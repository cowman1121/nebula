...
const TeacherDashboard = () => {
  ...
  const openSearchTab = () => {
    ...
    setOpenTabs([...openTabs, {id: newID, label: "Search Bar", type: "static"}]);
    setActiveTab(newID);
  };
  ...
  return (
    ...
    <div className="flex-1 bg-cream h-screen flex flex-col">   
      <div className="flex items-center border-t-2 border-baby-blue bg-blue-gray overflow-x-auto">
        <button 
        ...
        title="To-do" >
          ...
          {openTabs.length > 0 && <div className={divider} />}
          {openTabs.map((tab, index) => (
            <React.Fragment key={tab.id}>
                <button onClick={() => setActiveTab(tab.id)} className={`${tabStruct(tab.id)} flex shrink truncate items-center gap-5`} title={tab.label}>
                  {tab.label}
                  <X  onClick={(e) => {
                    e.stopPropagation();
                    closeTab(tab.id);
                   }} 
                  className="w-3 h-3 hover:bg-gray hover:rounded-lg hover:shadow-md"/>
                </button>
              {index < openTabs.length - 1 && <div className={divider} />}
            </React.Fragment>
